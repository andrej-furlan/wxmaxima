...
void Configuration::ReadStyles(const wxString &file)
{
  ...
  if (config->Read(wxT("mathfontsize"), &mathFontSize) && mathFontSize > 0)
    m_styles[TS_MATH].SetFontSize(mathFontSize);
...
Style Configuration::GetDefaultStyleAt(double fontSize) const
{
...
Style Configuration::GetStyle(TextStyle textStyle, GetStyleOpt options, double fontSize) const
{
  Style style = m_styles[textStyle];
  if (fontSize == -1 || IsIn(textStyle, CustomFontStyles))
