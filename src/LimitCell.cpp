...
LimitCell::LimitCell(Cell *parent, Configuration **config) :
  Cell(parent, config),
...
void LimitCell::SetName(OwningCellPtr name)
{
  if (!name)
    return;
...
void LimitCell::SetBase(OwningCellPtr base)
{
  if (!base)
    return;
...
void LimitCell::SetUnder(OwningCellPtr under)
{
  if (!under)
    return;
