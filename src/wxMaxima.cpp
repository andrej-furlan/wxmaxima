...
void wxMaxima::ConfigChanged()
{
  ...
  if (!m_worksheet->m_currentFile.empty())
  {
    SetCWD(m_worksheet->m_currentFile);
  }
  ...
  m_parser(&m_worksheet->m_configuration, &m_worksheet->m_cellPointers)
{
  ...
  m_oldFindFlags = 0;
  m_worksheet->m_currentFile.clear();
...
void wxMaxima::SendMaxima(const wxString &str, bool addToHistory)
{
  ...
  int index;
  wxString parenthesisError = GetUnmatchedParenthesisState(str, index);
  if (parenthesisError.empty())
  {
    wxString s = m_worksheet->UnicodeToMaxima(str);
...
{
  if((m_currentOutputEnd.empty()) || (m_currentOutputEnd.Find(tag) != wxNOT_FOUND))
...
void wxMaxima::ReadStatusBar(wxString &data)
{
  ...
  if ((end = FindTagEnd(data,m_statusbarSuffix)) != wxNOT_FOUND)
  {
    ...
    data.Remove(0, end + m_statusbarSuffix.Length());
  }
}
...
void wxMaxima::ReadLoadSymbols(wxString &data)
{
  ...
  if (end != wxNOT_FOUND)
  {
    ...
    data.Remove(0, end + m_symbolsSuffix.Length());
  }
}
...
void wxMaxima::ReadVariables(wxString &data)
{
  ...
  if (end != wxNOT_FOUND)
  {
    ...
    // Remove the symbols from the data string
    data.Remove(0, end + m_variablesSuffix.Length());
    TriggerEvaluation();
    QueryVariableValue();
  }
}
...
void wxMaxima::ReadAddVariables(wxString &data)
{
  ...
  if (end != wxNOT_FOUND)
  {
    ...
    data.Remove(0, end + m_addVariablesSuffix.Length());
  }
}
...
void wxMaxima::ReadPrompt(wxString &data)
{
  ...
  data.Remove(0, end + m_promptSuffix.Length());
  if(data == wxT(" "))
    data.clear();
...
void wxMaxima::CompileHelpFileAnchors()
{
  ...
  if(m_worksheet->m_helpFileAnchors.empty() && (!(MaximaHelpFile.empty())))
  {
  ...
  if(m_worksheet->m_helpFileAnchors["%solve"].empty())
...
void wxMaxima::ShowMaximaHelp(const wxString &text)
{
  wxString str;
  ...
  if((!(MaximaHelpFile.Lower().EndsWith(wxT(".html")))) && (!MaximaHelpFile.empty()))
  {
    ShowHTMLHelp(MaximaHelpFile,str);
  }
  else
  {
    wxBusyCursor crs;
    CompileHelpFileAnchors();
    str = m_worksheet->m_helpFileAnchors[str];
    if(str.empty())
      str = "Function-and-Variable-Index";
    if(!MaximaHelpFile.empty())
    {
      // A Unix absolute path starts with a "/", so a valid file URI
      // file:///path/to/helpfile (3 slashes!!) is constructed.
      // On Windows the path starts e.g. with C:/path/to/helpfile
      ...
      wxString maximaHelpfileURI = wxString("file://")+
                                   ...
                                   MaximaHelpFile;
      if(!str.empty()) {
        maximaHelpfileURI = maximaHelpfileURI + "#" + str;
      }
    ...
    else
    {
      ...
      wxLaunchDefaultBrowser("http://maxima.sourceforge.net/docs/manual/maxima_singlepage.html#"+str);
    }
  }
}
...
void wxMaxima::OnIdle(wxIdleEvent &event)
{
  ...
  if ((m_worksheet != NULL) && (m_worksheet->m_configuration->GetDC() != NULL))
  {
    ...
    if(!m_initialWorkSheetContents.empty())
    {
      //  Convert the comment block to an array of lines
      ...
      m_initialWorkSheetContents.clear();
    }
  }
...
void wxMaxima::ReadStdErr()
{
  ...
  if (m_process->IsInputAvailable())
  {
    ...
    if ((!o_trimmed.empty()) && (!o.StartsWith("Connecting Maxima to server on port")) &&
        (!m_first))
    {
  ...
  if (m_process->IsErrorAvailable())
  {
    ...
    if((o != wxT("Message from maxima's stderr stream: End of animation sequence")) &&
       !o.Contains("frames in animation sequence") && (!o_trimmed.empty()) &&
       (o.Length() > 1))
    {
...
bool wxMaxima::AutoSave()
{
  ...
  if (m_worksheet->m_configuration->AutoSaveAsTempFile() ||
      m_worksheet->m_currentFile.empty())
  {
  ...
  switch (event.GetId())
  {
    ...
    case menu_fun_def:
      cmd = GetTextFromUser(_("Show the definition of function:"),
                            _("Function"), m_worksheet->m_configuration, {}, this);
      if (cmd.Length())
      {
...
void wxMaxima::AddDrawParameter(const wxString &cmd, int dimensionsOfNewDrawCommand)
{
  if(!m_drawPane)
    return;
...
void wxMaxima::HelpMenu(wxCommandEvent &event)
{
  ...
  switch (event.GetId())
  {
    case wxID_ABOUT:
    {
      ...
      if (!m_maximaVersion.empty())
        description += _("\nMaxima version: ") + m_maximaVersion + " ("+m_maximaArch+")";
      else
        description += _("\nNot connected.");
      if (!m_lispVersion.empty())
  ...
  switch (event.GetId())
  {
    ...
    case menu_stats_subsample:
    {
      ...
      if (wiz->ShowModal() == wxID_OK)
      {
        ...
        if (!wiz->GetValue2().empty())
          cmd += wiz->GetValue2() + wxT(" ))");
        else
          cmd += wxT("true ))");

        if (!wiz->GetValue3().empty())
          cmd += wxT(",\n   ") + wiz->GetValue3();
...
void wxMaxima::TriggerEvaluation()
{
  ...
  if ((!text.empty()) && (text != wxT(";")) && (text != wxT("$")))
  {
    int index;
...
void wxMaxima::InsertMenu(wxCommandEvent &event)
{
  ...
  switch (event.GetId())
  {
    ...
    case Worksheet::popid_add_watch:
    {
      ...
      if(selectionString.empty() && (m_worksheet->GetSelectionStart() != NULL))
